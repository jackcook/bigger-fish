...
from requests.exceptions import ConnectionError, ContentDecodingError, ReadTimeout, TooManyRedirects
...
parser.add_argument("--browser", type=Browser, choices=list(Browser), default="chrome", help="The browser to use for the victim process.")
parser.add_argument("--num_runs", type=int, default=100)
parser.add_argument("--attacker_type", type=str, choices=["javascript", "javascript_cache", "counter", "ebpf"], default="counter")
parser.add_argument("--javascript_attacker_type", type=str, choices=["ours", "cache"], default="ours")
parser.add_argument("--trace_length", type=int, default=15, help="The length of each recorded trace, in seconds.")
parser.add_argument("--sites_list", type=str, default="alexa100", help="The list of sites to use. If using top n U.S. Alexa sites, should be alexan, where n >= 1.")
parser.add_argument("--receiver_ip", type=str, default="0.0.0.0", help="The address of the receiver, if we're using one.")
parser.add_argument("--receiver_port", type=int, default=1234, help="The port of the receiver, if we're using one.")
parser.add_argument("--out_directory", type=str, default="data", help="The output directory name.")
parser.add_argument("--timer_resolution", type=float, default=None, help="Resolution of the timer during counter attacks. e.g. if set to 0.001, rounds to nearest millisecond.")
parser.add_argument("--enable_timer_jitter", type=bool, default=False, help="True if we want to enable Chrome's jitter algorithm during counter attacks.")
parser.add_argument("--twilio_interval", type=float, default=0.1, help="Interval at which to send updates with Twilio. e.g. if set to 0.1, will send a text each time another 10%% of the traces are collected. Set to 0 to disable.")
parser.add_argument("--overwrite", type=bool, default=False, help="True if we want to overwrite the output directory.")
parser.add_argument("--disable_chrome_sandbox", type=bool, default=False, help="True if we want to disable Chrome's sandbox. browser must be set to chrome or chrome_headless.")
parser.add_argument("--tor_browser_path", type=str, help="Path to the Tor Browser bundle. browser must be set to tor_browser.")
parser.add_argument("--tor_onion_address", type=str, help="Onion address to use to access attacker over Tor.")
parser.add_argument("--enable_cache_countermeasure", type=bool, default=False, help="True if we want to enable the cache countermeasure Chrome extension.")
parser.add_argument("--enable_interrupts_countermeasure", type=bool, default=False, help="True if we want to enable the interrupts countermeasure Chrome extension.")
parser.add_argument("--enable_timer_countermeasure", type=bool, default=False, help="True if we want to enable the randomized timer countermeasure.")
parser.add_argument("--chrome_binary_path", type=str, default=None, help="Path to a directory containing chrome and chromedriver binaries, if desired.")
parser.add_argument("--ebpf_ns_threshold", type=int, default=500, help="Minimum gap duration to record interrupts with eBPF tool.")
...
if opts.disable_chrome_sandbox and opts.browser != Browser.CHROME and opts.browser != Browser.CHROME_HEADLESS:
    print("You can't set disable_chrome_sandbox to true unless browser is chrome or chrome_headless.")
    sys.exit(1)

if opts.browser == Browser.REMOTE and (opts.receiver_ip is None or opts.receiver_port is None):
...
if (opts.tor_browser_path is not None or opts.tor_onion_address is not None) and opts.browser != Browser.TOR_BROWSER:
    print("You can't set tor_browser_path or tor_onion_address without setting browser to tor_browser.")
    sys.exit(1)

if opts.browser == Browser.TOR_BROWSER and (opts.tor_browser_path is None or opts.tor_onion_address is None):
    print("If browser is tor_browser, tor_browser_path and tor_onion_address must be set.")
    sys.exit(1)
...
if opts.timer_resolution is not None and not os.path.exists(os.path.join("lib", "libtimer.so")):
...
def get_driver(browser):
    ...
    driver_cls = getattr(webdriver, browser.name[0] + browser.name[1:].split("_")[0].lower())

    if browser == Browser.CHROME or browser == Browser.CHROME_HEADLESS:
        chrome_opts = Options()
        
        if opts.chrome_binary_path is not None:
            chrome_opts.binary_location = os.path.join(opts.chrome_binary_path, "chrome")
...
if opts.twilio_interval != 0:
    if os.path.exists("twilio.json"):
        ...
        print("WARNING: No twilio.json file found, but twilio_interval > 0. Continuing anyway.")

def send_notification(message):
    if using_twilio:
        twilio_client.messages.create(
            body=message,
            from_=twilio_data["from"],
            to=twilio_data["to"]
        )
...
if opts.attacker_type == "javascript" or opts.attacker_type == "javascript_cache" or opts.attacker_type == "sleep":
    ...
    if opts.browser != Browser.SAFARI:
        ...
        attacker_browser.execute_script(f"window.trace_length = {opts.trace_length * 1000}")
...
def collect_data(q):
    ...
    if opts.attacker_type == "counter":
        while True:
            ...
            data[idx] = num
    elif opts.attacker_type == "javascript" or opts.attacker_type == "javascript_cache" or opts.attacker_type == "sleep":
        try:
            js_attacker_type = ("ours_cm" if opts.enable_timer_countermeasure else "ours") if opts.attacker_type == "javascript" else ("sleep" if opts.attacker_type == "sleep" else "cache")
            attacker_browser.execute_script(f"window.collectTrace(\"{js_attacker_type}\")")
        ...
        child = subprocess.run(["sudo", "ebpf/target/release/ebpf", f"--timeout={opts.trace_length * 1000}", f"--ns-threshold={opts.ebpf_ns_threshold}"], capture_output=True)
        data = []
...
with tqdm(total=total_traces) as pbar:
    ...
    def post_trace_collection():
        ...
        if using_twilio and traces_collected >= last_notification + notify_interval:
            send_notification(f"{twilio_data['name']} is done with {traces_collected / total_traces * 100:.0f}% of its current job!")
    ...
    for i, domain in enumerate(domains):
        ...
        if opts.browser == Browser.SAFARI and (opts.attacker_type == "javascript" or opts.attacker_type == "javascript_cache") and browser is not None:
